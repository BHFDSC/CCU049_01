...
# MAGIC %md
# MAGIC # Survival analysis - Death in TWO years
# MAGIC
...
# MAGIC %run /Workspaces//SHDS/Mehrdad/helper_library/date_management

# COMMAND ----------

death = spark.sql(f"SELECT DEC_CONF_NHS_NUMBER_CLEAN_DEID, REG_DATE_OF_DEATH FROM .deaths_")
...
co = co.withColumnRenamed('date','lc_date')
...
co_yr = co.withColumn('one_yr_date', F.date_add(co['lc_date'], 365)).withColumn('two_yr_date', F.date_add(co['lc_date'], 730))
...
co_yr = co_yr.withColumn('followup_day',when(F.col('dod').isNull(),floor(datediff(col('date_today'),col("lc_date"))))
                                 .otherwise(floor(datediff(col('dod'), col("lc_date")))))
...
# MAGIC %run /Workspaces//SHDS/Mehrdad/helper_library/basic_functions

# COMMAND ----------

save_checkpoint(df_in=co_yr, database="", temp_df="ccu049_lc_death", global_view_name="ccu049")
...
hes_apc_archive = spark.sql(f'select * from .hes_apc_all_years_archive')
hes_apc_otr_archive = spark.sql(f'select * from .hes_apc_otr_all_years_archive')
hes_cc_archive = spark.sql(f'select * from .hes_cc_all_years_archive')
icnarc_archive = spark.sql(f'select * from .icnarc__archive')
...
co_hes_apc = co_hes_apc.select('id','lc_date','ADMIDATE')

co_hes_apc = co_hes_apc.filter(F.col('ADMIDATE')>=F.col('lc_date'))
co_hes_apc = co_hes_apc.filter(F.col('ADMIDATE')<'2023-02-01')

# COMMAND ----------

from pyspark.sql.functions import col, min as min_

co_hes_apc_date = co_hes_apc.withColumn('ADMIDATE', col("ADMIDATE").cast("date")).groupBy('id').agg(min_("ADMIDATE"))


# COMMAND ----------

co_apc = co.join(co_hes_apc_date, on = "id", how = "left")

# COMMAND ----------

import pyspark.sql.functions as F 
# First, create the date column that is one year ahead of Long Covid diagnosis
co_apc_two_yr = co_apc.withColumn('two_yr_date', F.date_add(co['lc_date'], 730))

# COMMAND ----------

# Second, change the two_yr_date to 2023-01-31 if the actual two-yr-date was later than that
...
co_apc_two_yr = co_apc_two_yr.withColumn('followup_day',when(F.col('first_admin').isNull(),floor(datediff(col('date_today'),col("lc_date"))))
                                 .otherwise(floor(datediff(col('first_admin'), col("lc_date")))))
...
co_apc_two_yr = co_apc_two_yr.select('id','two_yr_hos','two_yr_followup_day','two_yr_followup_mon')

# COMMAND ----------

save_checkpoint(df_in=co_apc_two_yr, database="", temp_df="ccu049_lc_hosp", global_view_name="ccu049")
